// patch.js v8 — GAS Cloud Sync + Drive Image Upload
// ✅ ดัก QuotaExceededError ที่ Storage.prototype.setItem
// ✅ อัปโหลดรูปขึ้น Google Drive แทน base64 ใน localStorage
...
(function () {
  ...
  // แปลง File object → base64 แล้วส่ง GAS → ได้ Drive URL กลับมา
  async function uploadImageToDrive(file) {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = async (e) => {
        try {
          showSync('กำลังอัปโหลดรูป...', 'upload');
          const base64 = e.target.result; // data:image/xxx;base64,...
          const result = await gasCall('uploadImage', {
            base64,
            filename: file.name,
            mimeType: file.type
          });
          if (result.ok) {
            showSync('อัปโหลดรูปสำเร็จ ✅', 'success');
            resolve(result.url);
          } else {
            showSync('อัปโหลดรูปล้มเหลว: ' + (result.error || '?'), 'error');
  ...
  // ─── INTERCEPT input[type=file] สำหรับรูปภาพ ────────────
  // วิธีนี้ดัก event ทุก file input ใน DOM รวมถึงที่สร้างทีหลัง
  document.addEventListener('change', async function (e) {
    const input = e.target;
    // เฉพาะ input[type=file] ที่รับรูปภาพ
    if (input.tagName !== 'INPUT' || input.type !== 'file') return;
    if (!input.accept || !input.accept.includes('image')) return;
    const file = input.files && input.files[0];
    if (!file || !file.type.startsWith('image/')) return;
    if (!getToken()) return; // ยังไม่ login

    try {
      // อัปโหลดรูปขึ้น Drive
      const driveUrl = await uploadImageToDrive(file);

      // หา element ที่ index.html ใช้แสดง/เก็บรูป
      // ลอง pattern ทั่วไป: data-* attribute, hidden input ใกล้เคียง, หรือ img preview
      const container = input.closest('tr, .form-group, .item-row, div, td') || document.body;

      // อัปเดต hidden input หรือ field ที่เก็บ URL รูป
      const picFields = container.querySelectorAll('input[name*="pic"], input[name*="img"], input[name*="image"], input[data-field*="pic"]');
      picFields.forEach(f => { f.value = driveUrl; f.dispatchEvent(new Event('input', { bubbles: true })); });

      // อัปเดต img preview ถ้ามี
      const previews = container.querySelectorAll('img.preview, img[data-preview], .img-preview img, img.product-img');
      previews.forEach(img => { img.src = driveUrl; });

      // ถ้า index.html เก็บใน window variable
      if (window._editingBarcode) window._editingBarcode.pic = driveUrl;
      if (window._currentItem)    window._currentItem.pic   = driveUrl;
      if (window._formData)       window._formData.pic      = driveUrl;

      // inject value กลับเข้า input เป็น URL (ไม่ใช่ base64)
      // ทำให้ index.html คิดว่า user เลือก "file" ที่มี URL แทน
      Object.defineProperty(input, '_driveUrl', { value: driveUrl, writable: true, configurable: true });

      console.log('[patch v8] รูปอัปโหลดแล้ว → Drive URL:', driveUrl);

      // dispatch custom event ให้ index.html รับรู้
      input.dispatchEvent(new CustomEvent('driveUploadDone', {
        bubbles: true,
        detail: { url: driveUrl, file }
      }));

    } catch (err) {
      console.error('[patch v8] uploadImage failed:', err);
    }
  }, true); // useCapture = true เพื่อดักก่อน index.html
  ...
  window.gasSync = {
    ...
    uploadImage: uploadImageToDrive  // expose ให้ index.html เรียกได้
  };
  ...
  console.log('[patch.js] v8 loaded — Drive Image Upload ready');
